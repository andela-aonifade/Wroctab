...
describe('Document API', () => {
  ...
  before((done) => {
    server
      ...
      .end((err, res) => {
        ...
        server
          .post('/users/login')
          .send({ email: 'anuonifade@gmail.com', password: 'password' })
          .end((err, res) => {
            adminUser = res.body;
            server
                .post('/users/login')
  ...
  describe('/GET Documents', () => {
    ...
    (done) => {
      server
        .get('/users/anuonifade/documents')
        .set('x-access-token', regUserData.token)
        .expect('Content-Type', /json/)
        .end((err, res) => {
          expect(res.status).toEqual(400);
